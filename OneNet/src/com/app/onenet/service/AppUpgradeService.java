...
import com.app.onenet.utils.DownloadUtils;
...
public class AppUpgradeService extends Service{
    ...
    private Handler mHandler = new Handler(){
        ...
        public void handleMessage(Message msg) {
            ...
            case DOWNLOAD_SUCCESS:
                Toast.makeText(getApplicationContext(), R.string.app_upgrade_download_success, Toast.LENGTH_LONG).show();
                install(destFile);
                break;
            case DOWNLOAD_FAIL:
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (Environment.getExternalStorageState().equals(android.os.Environment.MEDIA_MOUNTED)){
            destDir = new File(Environment.getExternalStorageDirectory().getPath() +Constants.APP_DIR+ Constants.UPDATE_PATH);
            if (destDir.exists()) {
            	
                File destFile = new File(destDir.getPath() + "/" + URLEncoder.encode(mDownloadUrl.substring(mDownloadUrl.lastIndexOf("/")+1)));
                if (destFile.exists() && destFile.isFile() && checkApkFile(destFile.getPath())) {
                    install(destFile);
                    stopSelf();
        ...
        public void run() {
            if (Environment.getExternalStorageState().equals(android.os.Environment.MEDIA_MOUNTED)){
                ...
                if (destDir.exists() || destDir.mkdirs()) {
                    destFile = new File(destDir.getPath() + "/" + URLEncoder.encode(mDownloadUrl.substring(mDownloadUrl.lastIndexOf("/")+1)));
                    if (destFile.exists() && destFile.isFile() && checkApkFile(destFile.getPath())) {
                        install(destFile);
                    } else {
                        try {
